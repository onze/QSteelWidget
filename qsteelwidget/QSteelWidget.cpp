...
unsigned long QSteelWidget::createOgreModel(QString meshName, QVector3D pos, QVector4D rot, bool involvesNewResources)
{
...
//	Ogre::Quaternion r = mEngine->camera()->camNode()->getOrientation();
	Steel::ModelId mid = mLevel->newOgreModel(convert(meshName), convert(pos), convert(rot), involvesNewResources);
	update();
	quickLog("");
...
QList<QString> QSteelWidget::getAgentsJsons(QList<unsigned long> ids)
{
	QList<QString> list = QList<QString>();
	for (QList<unsigned long>::iterator it = ids.begin(); it != ids.end(); ++it)
	{
		Steel::Agent *agent = mLevel->getAgent((Steel::AgentId) *it);
		Json::Value json;
		if (agent != NULL)
			json = agent->toJson();
		list.append(convert(json.toStyledString()));
	}
	return list;
}
...
bool QSteelWidget::linkAgentToOgreModel(unsigned long agentId, unsigned long modelId)
{
#ifdef DEBUG
	assert(mLevel!=NULL);
#endif
	return mLevel->linkAgentToModel((Steel::AgentId) agentId, Steel::MT_OGRE, (Steel::ModelId) modelId);
}
		...
		switch (e->button())
		{
			case Qt::LeftButton:
				if (mEngine->hasSelection())
				{
					...
					emit onAgentsDeselected();
				}
				mEngine->pickAgents(selection, e->x(), e->y());
				mEngine->setSelectedAgents(selection, true);
				if (mEngine->hasSelection())
				{
					// saved so that we know what to reset propeties to
					// TODO: save the whole json representation
